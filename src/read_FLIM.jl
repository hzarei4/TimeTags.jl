function read_flim(filename; sx = 350, data_channel=2, marker_channel_y = nothing, marker_channel_z = nothing, bins=128, dtype=UInt32, is_bidirectional=true, tag_offset=0.0)
    q = read_ptu(filename);
    if length(q) != 3
